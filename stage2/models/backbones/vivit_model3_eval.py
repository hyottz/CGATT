...
class ViViTBackbone(nn.Module):
    ...
    def __init__(self, t, h, w, patch_t, patch_h, patch_w, num_classes, dim, depth, heads, mlp_dim, dim_head=3,
        ...
        # repeat same spatial position encoding temporally -> repeat 이라 deepcopy 불가능
        # self.pos_embedding = nn.Parameter(torch.randn(1, 1, self.nh * self.nw, dim)).repeat(1, self.nt, 1, 1)

        # 원래 ---------------------
        # 초기 pos_embedding을 생성합니다.
        # init_pos_embedding = torch.randn(1, 1, self.nh * self.nw, dim)

        # # expand를 사용하여 차원을 확장합니다.
        # expanded_pos_embedding = init_pos_embedding.expand(-1, self.nt, -1, -1)

        # # 확장된 텐서를 nn.Parameter로 변환하여 할당합니다.
        # self.pos_embedding = nn.Parameter(expanded_pos_embedding)

        # ----------------------------

        # 변경 ---------------------
        # pos_embedding 초기화 시 `expand` 대신 `repeat` 사용
        init_pos_embedding = torch.randn(1, 1, self.nh * self.nw, dim)
        repeated_pos_embedding = init_pos_embedding.repeat(1, self.nt, 1, 1)
        self.pos_embedding = nn.Parameter(repeated_pos_embedding)
    ...
    def forward(self, x):
        ...
        # print('after tubelet tokens,shape:', tokens.shape)

        # pos_embedding -> cuda 로 옮김
        self.pos_embedding = self.pos_embedding.to(tokens.device)
        
        tokens += self.pos_embedding
        tokens = self.dropout(tokens)
        # print('tokens shape after adding positional embedding and applying dropout:', tokens.shape)

        x = self.transformer(tokens) # output dimension: [b, nt*nh*nw, dim]
        # print('output shape after transformer layer:', x.shape)
        
        # 차원 변경: [b, nt*nh*nw, dim] -> [b, nt, dim]
        # 여기서 각 튜블릿의 토큰들에 대해 공간적 차원을 평균내어 제거합니다.
        b, _, _ = x.shape
        x = x.view(b, self.nt, self.nh*self.nw, -1).mean(dim=2)  # 공간적 차원에 대한 평균
        # print('output shape after taking mean along spatial dimension:', x.shape)
        
        # x = x.mean(dim=1) #x.mean 은 x의 1번째 차원을 제거한다. 즉, [b, dim]의 형태로 만든다. 그리고 이것을 하는 이유는 x
        # print('output shape after taking mean along dimension 1:', x.shape)

        x = self.to_latent(x)
        # print('output shape after passing through to_latent:', x.shape)
        
        # 차원 변경
        x = x.permute(0, 2, 1)  # 차원 변경: [batch_size, features, temporal_dimension]
        # print('final output shape after permute:', x.shape)
