...
def main():
    ...
    for path in tqdm(Path(video_root).rglob("*.mp4")):
        ...
        target_dir = os.path.join(target_root, relpath[:-4] + ".avi")  # 수정된 부분: 최종 파일명 지정
        # 이미 처리된 파일 확인
        if os.path.exists(target_dir):  # 수정된 부분
            print(f"Skipped {target_dir}, already exists.")  # 처리된 파일이 있으면 메시지 출력하고 건너뜀
            continue
        ...
        os.makedirs(os.path.dirname(target_dir), exist_ok=True)  # 폴더 생성 로직을 이 위치로 이동
        ...
        save_video_lossless(target_dir[:-4], sequence, 25, audio_path)  # 파일 이름 수정: 확장자 제거
